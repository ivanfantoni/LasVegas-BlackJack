...
#------------------------------------------------------------------
    ...
    def backcanvas(self):
        self.bc = Canvas(self.master, height=650, width=950, bd=0)
        self.bc.place(x=0, y=0, anchor='nw')

        b_img= Image.open(self.colors[0])
        b_r = b_img.resize((950, 600), Image.ANTIALIAS)
        b_image = ImageTk.PhotoImage(image=b_r)
        self.bc.create_image(0, 0, anchor='nw', image=b_image)
